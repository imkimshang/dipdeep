-- give_welcome_credits 함수 수정
-- auth.users 테이블 접근 권한 문제 해결
...
BEGIN
  ...
  -- auth.users는 SECURITY DEFINER로 접근 가능
  ...
  -- auth.users의 created_at 기준으로 순서 확인
...
-- BEGIN
--   -- 테스트용 프로필 삽입 (실제로는 INSERT하지 않음)
--   RAISE NOTICE '함수 컴파일 성공';
-- END $$;
